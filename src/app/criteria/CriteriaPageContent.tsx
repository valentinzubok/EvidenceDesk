...
import { AiCriteriaAssistant } from "@/components/AiCriteriaAssistant";
import {
  Alert,
  EmptyState,
  GlassCard,
...
export default function CriteriaPageContent() {
  ...
  const allTags = useMemo(() => {
    ...
    templates.forEach((tpl) => tpl.tags?.forEach((tag) => set.add(tag)));
    return [...set].sort();
  }, [templates]);
                ...
                <button
                  ...
                  className={`group h-full w-full rounded-xl border p-4 text-left transition-all duration-200 ${
                    selectedId === item.id
                      ...
                      : "border-white/5 surface-deep hover:border-teal-500/25 hover:bg-teal-500/5"
                  }`}
                >
      ...
      {selectedId ? (
        <GlassCard className="animate-fade-up space-y-4">
          ...
          {bodyLoading ? (
            ...
            <pre className="surface-code max-h-96 overflow-auto rounded-xl border border-white/5 p-4 text-xs leading-relaxed whitespace-pre-wrap">
